...
def mock_backend() -> MagicMock:
    ...
    backend.tools = [
        Tool(name=f"tool_{i}", description=f"Tool {i}", inputSchema={}) for i in range(50)
    ]
...
class TestBackendManagerBenchmarks:
    ...
    def test_get_all_tools_performance(
        self, benchmark: BenchmarkFixture, mock_backend: MagicMock
    ) -> None:
        ...
        for i in range(10):
            ...
            backend.is_connected = True
            backend.tools = [
                Tool(name=f"tool_{j}", description=f"Tool {j}", inputSchema={}) for j in range(50)
            ]
    ...
    def test_extract_tool_name_performance(self, benchmark: BenchmarkFixture) -> None:
...
class TestRateLimiterBenchmarks:
    ...
    def test_rate_limiter_check_performance(self, benchmark: BenchmarkFixture) -> None:
...
class TestConfigValidationBenchmarks:
    ...
    def test_server_config_validation(self, benchmark: BenchmarkFixture) -> None:
    ...
    def test_path_security_check(self, benchmark: BenchmarkFixture) -> None:
...
class TestCircuitBreakerBenchmarks:
    ...
    def test_circuit_breaker_state_transitions(self, benchmark: BenchmarkFixture) -> None:
