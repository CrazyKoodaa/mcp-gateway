...
class TestConfigChange:
    ...
    def test_added_change(self):
        ...
        change = ConfigChange(
            action="added", server_name="new-server", new_config={"command": "npx", "args": ["pkg"]}
        )
    ...
    def test_removed_change(self):
        ...
        change = ConfigChange(
            action="removed", server_name="old-server", old_config={"command": "npx"}
        )
...
class TestConfigWatcher:
    ...
    def temp_config_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            config = {
                ...
                "mcpServers": {"memory": {"command": "npx", "args": ["memory-server"]}},
            }
            json.dump(config, f)
            temp_path = f.name
        yield temp_path
    ...
    async def test_initialization_polling(self, temp_config_file, mock_reload_callback):
        ...
        watcher = ConfigWatcher(
            temp_config_file, mock_reload_callback, use_polling=True, poll_interval=1.0
        )
    ...
    async def test_start_stop_polling(self, temp_config_file, mock_reload_callback):
        ...
        watcher = ConfigWatcher(
            temp_config_file, mock_reload_callback, use_polling=True, poll_interval=0.1
        )
...
class TestHotReloadManager:
    ...
    def temp_config_file(self):
        """Create a temporary config file."""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            config = {"gateway": {"host": "127.0.0.1"}, "mcpServers": {}}
            json.dump(config, f)
            temp_path = f.name
        yield temp_path
        Path(temp_path).unlink(missing_ok=True)
    ...
    @pytest.fixture
    def reload_manager(
        self, temp_config_file, mock_backend_manager, mock_config_loader, mock_reconnect_callback
    ):
