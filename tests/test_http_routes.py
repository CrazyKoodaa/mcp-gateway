...
def mock_dependencies(tmp_path):
    ...
    backend_manager.backends = {
        "memory": MockBackend(
            "memory",
            connected=True,
            tools=[MockTool("add_memory", "Add a memory"), MockTool("get_memory", "Get memories")],
        ),
        "time": MockBackend(
            "time", connected=True, tools=[MockTool("get_current_time", "Get current time")]
        ),
    }
    ...
    metrics = MagicMock()
    metrics.generate_metrics = MagicMock(
        return_value="# HELP test metric\n# TYPE test gauge\ntest 1.0"
    )

    # Mock CircuitBreakerRegistry
    circuit_breaker_registry = MagicMock()
    circuit_breaker_registry.get = MagicMock(
    ...
    circuit_breaker_registry.get_all_stats = MagicMock(
        return_value={
            "memory": {
                "name": "memory",
                "state": "CLOSED",
...
class TestServerRoutes:
    ...
    def test_create_server(self, client, mock_dependencies):
        ...
        payload = {
            "name": "new-server",
            "config": {"command": "npx", "args": ["-y", "new-package"]},
        }
    ...
    def test_create_server_already_exists(self, client, mock_dependencies):
        ...
        payload = {"name": "memory", "config": {"command": "npx"}}
        response = client.post("/api/servers", json=payload)
    ...
    def test_update_server(self, client, mock_dependencies):
        ...
        payload = {"command": "uvx", "args": ["updated-package"]}
    ...
    def test_update_server_requires_approval(self, client, mock_dependencies):
        ...
        payload = {"command": "npx", "args": ["-y", "new-package", "/sensitive/path"]}
        response = client.put("/api/servers/memory", json=payload)
...
class TestAccessControlRoutes:
    ...
    def test_approve_access_request(self, client, mock_dependencies):
        ...
        mock_dependencies.access_control.approve_request = AsyncMock(
            return_value=(
                True,
...
class TestConfigApprovalRoutes:
    ...
    def test_approve_config_change(self, client, mock_dependencies):
        ...
        mock_dependencies.config_approval.approve = AsyncMock(
            return_value=(
                True,
                "Approved",
                ConfigChangeGrant(
    ...
    def test_approve_config_change_rate_limited(self, client, mock_dependencies):
        ...
        mock_dependencies.rate_limiter.check = AsyncMock(
            return_value=MagicMock(allowed=False, retry_after=60.0)
        )
