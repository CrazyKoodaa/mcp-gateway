...
class TestConfigPersistence:
    ...
    def test_config_persists_after_server_add(self, client_with_hot_reload, test_app_with_hot_reload):
        """Test that config is saved after adding server."""
        _, _, _, config_path = test_app_with_hot_reload
        
        # Add a new server
        response = client_with_hot_reload.post("/api/servers", json={
            "name": "new-server",
            "config": {
                "command": "npx",
                "args": ["-y", "test-package"]
    ...
    def test_config_persists_after_server_update(self, client_with_hot_reload, test_app_with_hot_reload):
        """Test that config is saved after updating server."""
        _, _, _, config_path = test_app_with_hot_reload
        
        # Update server
        response = client_with_hot_reload.put("/api/servers/time", json={
            "command": "uvx",
            "args": ["mcp-server-time", "--new-flag"],
            "disabledTools": []
        })
    ...
    def test_config_persists_after_server_delete(self, client_with_hot_reload, test_app_with_hot_reload):
...
class TestAtomicWrites:
    ...
    def test_no_temp_files_left_behind(self, client_with_hot_reload, test_app_with_hot_reload, tmp_path):
        ...
        for i in range(5):
            client_with_hot_reload.post("/api/servers", json={
                "name": f"temp-server-{i}",
    ...
    def test_config_valid_json_after_write(self, client_with_hot_reload, test_app_with_hot_reload):
        ...
        for i in range(10):
            client_with_hot_reload.post("/api/servers", json={
                "name": f"rapid-server-{i}",
                "config": {"command": "echo", "args": [str(i)]}
            })
...
class TestSearxngHotReload:
    ...
    def test_searxng_env_vars_preserved(self, client_with_hot_reload, test_app_with_hot_reload):
        ...
        client_with_hot_reload.put("/api/servers/memory", json={
            "command": "npx",
