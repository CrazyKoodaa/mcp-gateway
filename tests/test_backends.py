...
class TestBackendConnection:
    ...
    def sse_config(self):
        ...
        return ServerConfig(name="sse-server", url="http://localhost:8001/sse", type="sse")
    ...
    async def test_connect_stdio_success(self, stdio_config):
        ...
        with (
            patch("mcp_gateway.backends.stdio_client") as mock_stdio_client,
    ...
    async def test_connect_remote_streamable_http(self, remote_config):
        ...
        with (
            patch("mcp_gateway.backends.streamablehttp_client") as mock_http_client,
            patch("mcp_gateway.backends.ClientSession") as mock_client_session,
        ):
            mock_transport = (AsyncMock(), AsyncMock())
    ...
    async def test_connect_remote_sse(self, sse_config):
        ...
        with (
            ...
            assert backend.is_connected is True
            mock_sse_client.assert_called_once_with(url="http://localhost:8001/sse", headers={})
    ...
    def test_filter_tools_empty_disabled(self, stdio_config):
        ...
        tools = [Tool(name="tool1", description="Tool 1", inputSchema={})]
...
class TestBackendManager:
    ...
    async def test_connect_all_with_failure(self, manager):
        ...
        with patch.object(
            BackendConnection, "connect", AsyncMock(side_effect=[None, Exception("Failed")])
        ):
            await manager.connect_all(configs)
