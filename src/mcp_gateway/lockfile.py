...
This could mean:
  ...
  - Another service is using port {port}

Options:
  1. Find and stop the process:  lsof -ti:{port} | xargs kill -9
  2. Use a different port:       mcp-gateway --port {port + 1}
  3. Check what's using it:      lsof -i:{port}
